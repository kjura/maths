...
\paragraph{1.}
\(A\) i \(B\) nie jest puste bo \(A\) ma elementy przynajmniej do \(a\) które jest mniejsze niż \(c\), a \(B\) nie jest puste bo ma elementy przynajmniej aż do \(b\)
które jest większe niż \(c\)

\paragraph{2.}
negujemy "dla dowolnego c w przedziale (a,b) c w C", czyli "istnieje c w tym przedziale ktory nie jest w C"
...
\paragraph{4.}
Tu nie jestem pewny, ale dlatego, że jak mamy jakieś zbiory \(A, B\), takie, że \(A \subset B\) to domknięcie zachowuje zawieranie?
Czyli wtedy \(\overline{A} \subset \overline{B}\)?
...
i z czego tu korzystamy, że:

\begin{equation*}
    \overline{A} \cap B = \emptyset \ \ \mbox{?}
\end{equation*}

\paragraph{6.}

Wydaje mi się, że dlatego, że
