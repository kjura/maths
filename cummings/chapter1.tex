...
\end{equation*}
because there are no two integers obeying the above.
\subsection*{(c)}
...
Since \(n^2 \mid n\) we can write it as \(n = n^2 \cdot k\) for some integer
\(k\). We then have:
\begin{equation*}
	0 = n^{2}k - n = n(nk - 1)
\end{equation*}
Either \(n = 0\) or \(nk - 1 = 0\). But the second case means that \(nk = 1\) and this can
only happen if \(n = 1\) and \(k = 1\) or  \(n = -1\) and \(k = -1\)
\end{document}
