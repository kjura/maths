...
when \(\overrightarrow{HB}\) and \(\overrightarrow{EC}\) intersect. We are working in the standard basis
so we can quickly deduce that:
\begin{equation*}
    \overrightarrow{EC} = [2, 1, -1]
\end{equation*}
\begin{equation*}
    \overrightarrow{HB} = [1, -2, -1]
\end{equation*}
 
Now we need to calculate the dot product between the two:
\begin{equation*}
    |\overrightarrow{EC}| |\overrightarrow{HB}| \cos(\theta) = \overrightarrow{EC} \cdot \overrightarrow{HB}
\end{equation*}
\begin{equation*}
    \overrightarrow{EC} \cdot \overrightarrow{HB} = 2 + (-1) + (-2) = -1
\end{equation*}
\begin{equation*}
    |\overrightarrow{EC}| = \sqrt{4 + 1 + 1} = \sqrt{6}
\end{equation*}
\begin{equation*}
    |\overrightarrow{HB}| = \sqrt{1 + 4 + 1} = \sqrt{6}
\end{equation*}

We are interested in \(\theta\) so:
\begin{equation*}
    \cos(\theta) = \frac{-1}{\sqrt{6}\sqrt{6}} = \frac{-1}{6}
\end{equation*}
\begin{equation*}
    \theta = \cos^{-1}(\frac{-1}{6}) \approx 1.74
\end{equation*}
