...
The proof is simple for cases \(x = 0, y \neq 0\) and \(x \neq 0, y = 0\). For the former:
\begin{equation*}
	x^{2} + \alpha xy + y^{2} = 0 + \alpha \cdot 0 \cdot y + y^{2} = y^{2} > 0 
\end{equation*}
and for the later:
\begin{equation*}
	x^{2} + \alpha xy + y^{2} = x^{2} + \alpha \cdot x \cdot 0 + 0^{2} = x^{2} > 0 
\end{equation*}
so in these cases \(\alpha \in \mathbb{R}\). Assume now \(x \neq 0, y \neq 0\). Let \(b = \alpha y\)
and \(c = y^{2}\). We check for a negative expression:
\begin{equation*}
	b^{2} - 4c = \alpha^{2}y^{2} - 4y^{2} = y^{2}(\alpha^{2} - 4) = y^{2}(\alpha - 2)(\alpha + 2)
\end{equation*}
There are no solutions for the case \(\alpha < -2\) and \(\alpha > 2\) but we can find a valid interval
to make the term above negative i.e we can take \(\alpha > -2\) and \(\alpha < 2\) so for
\(\alpha \in (-2, 2)\) we have \(\alpha^{2}y^{2} - 4y^{2} < 0\). Using (18.b):
\begin{equation*}
	x^{2} + \alpha xy + y^{2} = x^{2} + bx + c > 0
\end{equation*}
what we inteded to show.


\subsection*{(e)}

Consider first \(x^{2} + bx + c\). Completing the square yields:
\begin{equation*}
	x^{2} + bx + c = \underbrace{(x + \frac{b}{2})^{2}}_\text{\(\geq\) 0} + \underbrace{c - \frac{b^{2}}{4}}_\text{constant}
\end{equation*}
And we can conclude that the only contributing factor that makes the expression bigger 
is \((x + \frac{b}{2})^{2}\). Since for any \(a \in \mathbb{R}\), \(a^{2} \geq 0\), we can obtain
the value of \(x\) by examining the left term when it is equal to zero:
\begin{equation*}
	x + \frac{b}{2} = 0
\end{equation*}
\begin{equation*}
	x = \frac{-b}{2}
\end{equation*}
So \(x = \frac{-b}{2}\) is the smallest possible value of \(x^{2} + bx + c\).
Consider now \(ax^{2} + bx + c\). Assume also that \(a > 0\). By repeating the above argument we get:
\begin{equation*}
	ax^{2} + bx + c = a(x^{2} + \frac{bx}{a} + \frac{c}{a})
\end{equation*}
\begin{equation*}
	= a\Bigg((x + \frac{b}{2a})^{2} - \frac{b^{2}}{4a^{2}} + \frac{c}{a}\Bigg)
\end{equation*}
\begin{equation*}
	= a\Bigg((x + \frac{b}{2a})^{2} + \frac{4ac}{4a^{2}} - \frac{b^{2}}{4a^{2}}\Bigg)
\end{equation*}
\begin{equation*}
	= a\Bigg((x + \frac{b}{2a})^{2} + \frac{4ac - b^{2}}{4a^{2}}\Bigg)
\end{equation*}
\begin{equation*}
	= a(x + \frac{b}{2a})^{2} + a \frac{4ac - b^{2}}{4a^{2}}
\end{equation*}
\begin{equation*}
	= a(x + \frac{b}{2a})^{2} + \frac{4a^{2}c - ab^{2}}{4a^{2}}
\end{equation*}
\begin{equation*}
	= \underbrace{a(x + \frac{b}{2a})^{2}}_\text{\(\geq\) 0} + \underbrace{\frac{4ac - b^{2}}{4a}}_\text{constant}
\end{equation*}
We again look what happens at zero:
\begin{equation*}
	x + \frac{b}{2a} = 0
\end{equation*}
\begin{equation*}
	x = - \frac{b}{2a}
\end{equation*}
Thus, \(x = - \frac{b}{2a}\) is the smallest possible value of \(ax^{2} + bx + c\).
...
Suppose \(x_{1} = \lambda y_{1}\) and \(x_{2} = \lambda y_{2}\) for some number \(\lambda \geq 0\).
We then have:
\begin{equation*}
	x_{1}y_{1} + x_{2}y_{2} = \lambda y_{1}y_{1} + \lambda y_{2}y_{2}
\end{equation*}
\begin{equation*}
	= \lambda (y_{1}^{2} + y_{2}^{2}) \leq \sqrt{x_{1}^{2} + x_{2}^{2}}\sqrt{y_{1}^{2} + y_{2}^{2}}
\end{equation*}
\begin{equation*}
	= \sqrt{\lambda^{2} y_{1}^{2} + \lambda^{2} y_{2}^{2}}\sqrt{y_{1}^{2} + y_{2}^{2}}
\end{equation*}
\begin{equation*}
	= \sqrt{\lambda^{2}}\sqrt{y_{1}^{2} + y_{2}^{2}}\sqrt{y_{1}^{2} + y_{2}^{2}}
\end{equation*}
\begin{equation*}
	= \sqrt{\lambda^{2}} (y_{1}^{2} + y_{2}^{2}) = |\lambda|(y_{1}^{2} + y_{2}^{2}) = \lambda (y_{1}^{2} + y_{2}^{2})
\end{equation*}
It follows that:
\begin{equation*}
	x_{1}y_{1} + x_{2}y_{2} = \sqrt{x_{1}^{2} + x_{2}^{2}}\sqrt{y_{1}^{2} + y_{2}^{2}}
\end{equation*}
Suppose now that \(y_{1} = y_{2} = 0\). Then:
\begin{equation*}
	x_{1}y_{1} + x_{2}y_{2} = x_{1} \cdot 0 + x_{2} \cdot 0 = 0 \leq \sqrt{x_{1}^{2} + x_{2}^{2}}\sqrt{y_{1}^{2} + y_{2}^{2}}
\end{equation*}
\begin{equation*}
	= \sqrt{x_{1}^{2} + x_{2}^{2}} \cdot \sqrt{0 + 0} = \sqrt{x_{1}^{2} + x_{2}^{2}} \cdot 0 = 0
\end{equation*}
It follows:
\begin{equation*}
	x_{1}y_{1} + x_{2}y_{2} = 0 = \sqrt{x_{1}^{2} + x_{2}^{2}}\sqrt{y_{1}^{2} + y_{2}^{2}}
\end{equation*}
Now, let us focus on the following inequality:
\begin{equation*}
...
\end{equation*}
We consider a quadratic formula with \(\lambda\) as an argument.
Since the whole quadratic is strictly bigger than \(0\), the discriminant must be negative so:
\begin{equation*}
	(-2)^{2}(x_{1}y_{1} + x_{2}y_{2})^{2} - 4(y_{1}^{2} + y_{2}^{2})(x_{1}^{2} + x_{2}^{2}) < 0
\end{equation*}
We have:
\begin{equation*}
	4(x_{1}y_{1} + x_{2}y_{2})^{2} < 4(y_{1}^{2} + y_{2}^{2})(x_{1}^{2} + x_{2}^{2})
\end{equation*}
\begin{equation*}
	(x_{1}y_{1} + x_{2}y_{2})^{2} < (y_{1}^{2} + y_{2}^{2})(x_{1}^{2} + x_{2}^{2})
\end{equation*}
Since for any \(a \in \mathbb{R}\) it is true that \(a^{2} \geq 0\), we can conclude that
\((x_{1}y_{1} + x_{2}y_{2})^{2} \geq 0\). Then \((y_{1}^{2} + y_{2}^{2})(x_{1}^{2} + x_{2}^{2}) > 0\)
and we can safely take a square root on both sides to get:
\begin{equation} \label{eq:1}
	x_{1}y_{1} + x_{2}y_{2} < \abs{x_{1}y_{1} + x_{2}y_{2}} < \sqrt{y_{1}^{2} + y_{2}^{2}} \sqrt{x_{1}^{2} + x_{2}^{2}}
\end{equation}
Consider two cases. Assume first WLOG that \(y_{1} = 0\) and \(y_{2} \neq 0\). Then (\ref{eq:1}) takes
the following form:
\begin{equation*}
	x_{2}y_{2} < \sqrt{y_{2}^{2}} \sqrt{x_{1}^{2} + x_{2}^{2}} = \sqrt{y_{2}^{2}x_{1}^{2} + y_{2}^{2}x_{2}^{2}}
\end{equation*}
For the final case, suppose that both \(y_{1} \neq 0\) and \(y_{2} \neq 0\). We go back to the original form of \((\ref{eq:1})\):
\begin{equation*}
	x_{1}y_{1} + x_{2}y_{2} < \sqrt{y_{1}^{2} + y_{2}^{2}} \sqrt{x_{1}^{2} + x_{2}^{2}}
\end{equation*}
and since this is a strict inequality, regardless of a case, we can obtain the final form:
\begin{equation*}
	x_{1}y_{1} + x_{2}y_{2} \leq \sqrt{y_{1}^{2} + y_{2}^{2}} \sqrt{x_{1}^{2} + x_{2}^{2}}
\end{equation*}

\subsection*{(b)}

Let us start first with showing that \(2xy \leq x^{2} + y^{2}\). We have:
\begin{equation*}
	(x - y)^{2} = x^{2} - 2xy + y^{2}
\end{equation*}
and since for any \(a \in \mathbb{R}\), \(a^{2} \geq 0\) we see that:
\begin{equation*}
...
\begin{equation*}
	2xy \leq x^{2} + y^{2}
\end{equation*}
We use this fact now with:

\begin{equation*}
	x = \frac{x_i}{\sqrt{x_{1}^{2} + x_{2}^{2}}}
\end{equation*}
\begin{equation*}
	y = \frac{y_i}{\sqrt{y_{1}^{2} + y_{2}^{2}}}
\end{equation*}
with \(i=1,2\). We substitute:
\begin{equation*}
	2 \frac{x_1}{\sqrt{x_{1}^{2} + x_{2}^{2}}} \frac{y_1}{\sqrt{y_{1}^{2} + y_{2}^{2}}} \leq \frac{x_{1}^{2}}{x_{1}^{2} + x_{2}^{2}} + \frac{y_{1}^{2}}{y_{1}^{2} + y_{2}^{2}}
\end{equation*}
\begin{equation*}
	2 \frac{x_2}{\sqrt{x_{1}^{2} + x_{2}^{2}}} \frac{y_2}{\sqrt{y_{1}^{2} + y_{2}^{2}}} \leq \frac{x_{2}^{2}}{x_{1}^{2} + x_{2}^{2}} + \frac{y_{2}^{2}}{y_{1}^{2} + y_{2}^{2}}
\end{equation*}
and add inequalities together to get:
\begin{equation*}
	\frac{2x_{1}y_{1} + 2x_{2}y_{2}}{\sqrt{x_{1}^{2} + x_{2}^{2}}\sqrt{y_{1}^{2} + y_{2}^{2}}} \leq \frac{x_{1}^{2} + x_{2}^{2}}{{x_{1}^{2} + x_{2}^{2}}} + \frac{y_{1}^{2} + y_{2}^{2}}{y_{1}^{2} + y_{2}^{2}} = 2
\end{equation*}
We can divide by \(2\) and multiply by the denominator of LHS to finally get Schwarz inequality:
\begin{equation*}
...
\end{equation*}
We are starting with \((x_{1}^{2} + x_{2}^{2})(y_{1}^{2} + y_{2}^{2})\) and expanding it:
\begin{equation*}
	x_{1}^{2}y_{1}^{2} + x_{1}^{2}y_{2}^{2} + x_{2}^{2}y_{1}^{2} + x_{2}^{2}y_{2}^{2} =
\end{equation*}
\begin{equation*}
	x_{1}^{2}y_{1}^{2} + x_{2}^{2}y_{2}^{2} + x_{1}^{2}y_{2}^{2} + x_{2}^{2}y_{1}^{2} =
\end{equation*}
We observe that we can add \(0\) to the expression and it - being an additive identity - will not
change the expression. We add \(0 = 2x_{1}y_{1}x_{2}y_{2} - 2x_{1}y_{1}x_{2}y_{2}\) to the expression:
\begin{equation*}
	x_{1}^{2}y_{1}^{2} + x_{2}^{2}y_{2}^{2} + 0 + x_{1}^{2}y_{2}^{2} + x_{2}^{2}y_{1}^{2} =
\end{equation*}
\begin{equation*}
	x_{1}^{2}y_{1}^{2} + x_{2}^{2}y_{2}^{2} + 2x_{1}y_{1}x_{2}y_{2} - 2x_{1}y_{1}x_{2}y_{2} + x_{1}^{2}y_{2}^{2} + x_{2}^{2}y_{1}^{2} =
\end{equation*}
\begin{equation*}
	\Big( x_{1}^{2}y_{1}^{2} + 2x_{1}y_{1}x_{2}y_{2} + x_{2}^{2}y_{2}^{2} \Big) + \Big( x_{1}^{2}y_{2}^{2} - 2x_{1}y_{1}x_{2}y_{2} + x_{2}^{2}y_{1}^{2} \Big) =
\end{equation*}
\begin{equation*}
	(x_{1}y_{1} + x_{2}y_{2})^{2} + (x_{1}y_{2} - x_{2}y_{1})^{2}
\end{equation*}
We use what we have just shown:
\begin{equation*}
	(x_{1}^{2} + x_{2}^{2})(y_{1}^{2} + y_{2}^{2}) = \underbrace{(x_{1}y_{1} + x_{2}y_{2})^{2}}_\text{\(\geq 0\)} + \underbrace{(x_{1}y_{2} - x_{2}y_{1})^{2}}_\text{\(\geq 0\)} \geq (x_{1}y_{1} + x_{2}y_{2})^{2}
\end{equation*}
...
\end{equation*}
\begin{equation*}
	\sqrt{x_{1}^{2} + x_{2}^{2}}\sqrt{y_{1}^{2} + y_{2}^{2}} \geq \abs{x_{1}y_{1} + x_{2}y_{2}} \geq x_{1}y_{1} + x_{2}y_{2}
\end{equation*}
and the proof is complete.
