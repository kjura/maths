...
natural numbers we can say that \(mn \in \mathbb{Z}\). Now assume that \(m < 0\) and \(n < 0\).
Let us write down for a fact, that now \(-m \in \mathbb{N}\) and \(-n \in \mathbb{N}\). Observe that:
\begin{equation*}
    mn = -\Big((-m) \cdot (-n)\Big)
\end{equation*}
by the closure under multiplication of natural numbers we know that \((-m) \cdot (-n)\) is in \(\mathbb{N}\) and that
implies that \(-\Big((-m) \cdot (-n)\Big) \in \mathbb{Z}\). Without loss of generality,
